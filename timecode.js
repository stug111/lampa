(function () {
  ...
  var Timecode = /*#__PURE__*/ (function () {
    function Timecode(field) {
      ...
      this.localhost = "https://bwa-cloud.apn.monster/";
        ...
        value: function url(method) {
          ...
          if (account.email) {
            url = Lampa.Utils.addUrlComponent(
              url,
              "account_email=" + encodeURIComponent(account.email)
            );
            if (account.profile)
              url = Lampa.Utils.addUrlComponent(
                url,
                "profile=" + encodeURIComponent(account.profile.id)
              );
          } else if (uid) {
            url = Lampa.Utils.addUrlComponent(
              url,
              "account_email=" + encodeURIComponent(uid)
            );
            url = Lampa.Utils.addUrlComponent(
              url,
              "profile=" + encodeURIComponent(uid)
            );
          }
          url = Lampa.Utils.addUrlComponent(
            url,
      ...
      },
      {
        ...
        value: function filename() {
          ...
          if (window.localStorage.getItem(name) === null && acc.profile) {
            Lampa.Storage.set(
              name,
              Lampa.Arrays.clone(Lampa.Storage.cache("file_view", 10000, {}))
            );
          }
          return name;
        },
